-- Re-crea RPCs de perfil con firma completa para evitar respuestas parciales
-- en entornos donde existía una versión antigua de las funciones.

DROP FUNCTION IF EXISTS public.get_my_profile_safe();
DROP FUNCTION IF EXISTS public.get_profile_safe(uuid);

CREATE FUNCTION public.get_profile_safe(p_user_id uuid DEFAULT NULL)
RETURNS TABLE (
  id uuid,
  full_name text,
  first_name text,
...
CREATE FUNCTION public.get_my_profile_safe()
RETURNS TABLE (
  id uuid,
  full_name text,
  first_name text,
